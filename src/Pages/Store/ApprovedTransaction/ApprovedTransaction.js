...
const ApprovedTransaction = () => {
  document.title = "Approved Transaction";
  ...
  const columns = [
    ...
    { field: "user_name", headerName: "Required From Name", width: 250 },
    { field: "transaction_id", headerName: "Required Ref ID", width: 280 },
    {
  ...
  const getPendingData = async () => {
    ...
    // console.log(data)
    if (data.code == 200) {
      const arr = data.response.data.map((row) => {
        return {
          ...row,
          id: v4(),
        };
      ...
      <div style={{ height: "90%" }}>
        ...
        <div style={{ height: "90%", margin: "10px" }}>
          <Skeleton loading={loading}>
            <MyDataTable data={allPending} columns={columns} />
          </Skeleton>
        </div>
        <NewModal
          setOpen={setOpen}
