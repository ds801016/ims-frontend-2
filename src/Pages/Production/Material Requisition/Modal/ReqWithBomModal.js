...
const ReqWithBomModal = ({ allBom, back, setTab, reset }) => {
  ...
  const [tabsExist, setTabsExist] = useState(["P", "PCK", "O"]);
  ...
  const getFetcgData = async () => {
    ...
    let arr = tableData;
    arr = [...dataArray, ...dataArray1, ...dataArray2];
    setTableData(arr);
  };
  ...
  const columns = [
    {
      ...
      renderCell: ({ row }) => (
        <span
          style={{ color: Number(row.leftQty) < Number(row.mfgQTY) && "red" }}
        >
          {row.name}
        </span>
      ),
  ...
  //             loading={loading}
  //             columns={columns}
  //             data={tableData.filter((row) => row.type == "part")}
  //           />
  //         </div>
  //       </div>
  //     ),
  ...
  //             loading={loading}
  //             columns={columns}
  //             data={tableData.filter((row) => row.type == "packing")}
  //           />
  //         </div>
  //       </div>
  //     ),
  ...
  //             loading={loading}
  //             columns={columns}
  //             data={tableData.filter((row) => row.type == "other")}
  //           />
  //         </div>
  //       </div>
  //     ),
