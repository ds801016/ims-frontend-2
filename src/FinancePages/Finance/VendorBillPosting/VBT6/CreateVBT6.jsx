  ...
  useEffect(() => {
    ...
    if (editingVBT?.length > 0) {
      ...
      setVendorData({
        ...
        bill_amount: "",
        vbt_gstin: "",
        comment: `Being jobwork charges on inv ${editingVBT[0]?.invoice_id}/ dt  of amt   tds --`,
      });
      setVBT(editingVBT[0]);
      ...
      let arr = editingVBT?.map((row) => {
        ...
        return {
          ...
          in_gst_sgst:
            row.in_gst_sgst == "--" ? 0 : +Number(row.in_gst_igst).toFixed(2),
  ...
  return (
    ...
    >
      ...
      <Row gutter={6} style={{ height: "93%" }}>
        <Col span={4} style={{ overflow: "auto", height: "100%" }}>
          <HeaderDetails
