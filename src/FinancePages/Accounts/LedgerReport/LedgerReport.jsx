...
export default function LedgerReport() {
  ...
  const downloadFun = () => {
    ...
    csvData = [
      { Ledger: searchLedger.label, Date: searchDateRange },
      ...csvData,
      {
        Opening: "Opening: " + ledgerData?.summary[0]?.opening?.toFixed(2),
        "Current Total Debit":
          "Current Total Debit: " +
          ledgerData?.summary[0]?.total_debit?.toFixed(2),
        "Current Total Credit":
          "Current Total Credit: " +
          ledgerData?.summary[0]?.total_credit?.toFixed(2),
        Closing: "Closing: " + ledgerData?.summary[0]?.closing?.toFixed(2),
      },
    ];
    // console.log(csvData);
    downloadCSVCustomColumns(
  ...
  useEffect(() => {
    if (params) {
      // getLedgerOptions(params.code);
      // let obj = { text: params.name, value: params.code };
      // console.log(obj);
      //
            ...
            <Row gutter={[0, 10]}>
              ...
              <Col span={24}>
                ...
                {!loading && ledgerData?.summary && (
                  ...
                  >
                    {ledgerData?.summary[0]?.opening?.toFixed(2)}
                  </Typography.Text>
                )}
              </Col>
              <Col span={24}>
              ...
              <Col span={24}>
                ...
                {!loading && ledgerData?.summary && (
                  ...
                  >
                    {ledgerData?.summary[0]?.closing?.toFixed(2)}
                  </Typography.Text>
                )}
              </Col>
            </Row>
